...
public class MuddyReforge extends ReforgeBase {
    @Override
    ...
    public @NotNull SkyblockReforge getReforge() {
        return SkyblockReforge.MUDDY;
    }

    @Override
    public @NotNull List<String> getDisplayStats() {
        return Arrays.asList(
                ChatColor.GOLD+"+3 "+ SkyblockConstants.FORTUNE + " Excavating Fortune",
    ...
    public void applyBaseStats(int modifier, @NotNull ItemStack item) {
        ...
        if(c.has(SkyblockD.getKey("excavatingFortune"), PersistentDataType.INTEGER)) {
            amount += Objects.requireNonNull(c.get(SkyblockD.getKey("excavatingFortune"), PersistentDataType.INTEGER));
        }
        c.set(SkyblockD.getKey("excavatingFortune"), PersistentDataType.INTEGER, amount);
        item.setItemMeta(m);
    }
    ...
    public void removeBaseStats(int modifier, @NotNull ItemStack item) {
        ...
        int amount = modifier * 3;
        Integer pre = c.get(SkyblockD.getKey("excavatingFortune"), PersistentDataType.INTEGER);
        assert pre != null;
        c.set(SkyblockD.getKey("excavatingFortune"), PersistentDataType.INTEGER, pre-amount);
        item.setItemMeta(m);
    }
}
